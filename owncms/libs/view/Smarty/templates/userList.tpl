...
<table border="1" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td>{if isset($list.url_name) && !empty($list.url_name)}<a href="/index.php?m=keywords&c=keywords&e=wordList&userID={$list.user_id}">关键词列表</a> | <a href="/index.php?m=keywords&c=keywords&e=wordsAdd&userID={$userBaseRes.user_id}">添加关键词</a> | {/if}<a href="/index.php?m=user&c=user&e=userUpdate&userID={$list.user_id}">修改</a> | <a href="/index.php?m=user&c=user&e=userDel&userID={$list.user_id}">删除</a> </td>
    </tr>
    {/foreach}
